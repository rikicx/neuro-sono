...
export const seedData = {
  users: [
    ...
    { id: "u4", name: "Lorrane Almeida", email: "—", role: "A definir", status: "Cadastro pendente", initials: "LA" },
    { id: "u5", name: "Paula Calmona", email: "—", role: "A definir", status: "Cadastro pendente", initials: "PC" },
  ],
  patients: [
...
export function loadPrototype() {
  ...
  try {
    ...
    const users = Array.isArray(parsed.users)
      ? parsed.users.map((user) => user.id === "u4" ? { ...user, name: "Lorrane Almeida", initials: "LA" } : user)
      : seedData.users;
    return { ...seedData, ...parsed, users };
